...
import {getContentfulAsset} from "./gql/fetchContentfulAssetById";
...
async function fetchContentfulAssetById(id) {
    const response = await apiCall(getContentfulAsset(id));
    const {data} = await response.json()
    return data?.asset
}
